...
from sklearn.preprocessing import StandardScaler # Importing the standrizer
...
if (selected == 'Developers salary prediction'):
    ...
    if st.button('Predict the salary'):
        dev_sal_predict = developers_salary_prediction.predict([[edu_level, Employment_system, experince_years, LanguageWorkedWith_Count]])                          
        
        #f'The predicted present car price is:{car_predict}in thousand USD ($)'
        
        devsalpredict = f'The Predicted Salary Is: {car_predict.round()}Thousands Dollars.'
